...
TILE_SIZE = 48  # Assuming each tile is 48x48 pixels
WORLD_WIDTH = 150 * TILE_SIZE  # Further adjusted to include all buttons
WORLD_HEIGHT = 80 * TILE_SIZE  # Further adjusted to include all buttons
...
class Camera:
    def __init__(self, width, height):
        ...
        self.x = 0
        self.y = 200  # Move camera up
        self.smoothness = 0.1  # Smoothing factor for gradual camera movement

    def apply(self, entity):
        return entity.rect.move(self.camera.topleft)

    def update(self, target):
        # Calculate target position (centering player)
        target_x = -target.rect.centerx + SCREEN_WIDTH // 2
        target_y = -target.rect.centery + SCREEN_HEIGHT + 600 // 2

        # Set world boundaries (adjust these values based on your level size)
        target_x = min(0, target_x)  # Left boundary
        target_x = max(-(WORLD_WIDTH - SCREEN_WIDTH), target_x)  # Right boundary
        target_y = min(0, target_y)  # Top boundary
        target_y = max(-(WORLD_HEIGHT - SCREEN_HEIGHT), target_y)  # Bottom boundary

        # Smoothly move camera in both x and y directions
        ...
        print(f"Camera target: {target_x}, {target_y}")
...
class InfoButton(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        self.rect.x = x
        # Move button down by 5 pixels
        self.rect.bottom = y + 9
        
        # Button state variables
        self.is_pressed = False
        self.cooldown = 0
        self.player_on_button = False
        self.is_button_pressed = False  # Renamed from button_pressed to avoid confusion
        self.already_activated = False  # Track if button was already used during this contact
    ...
    def update(self):
        ...
        # Debug button state
        if self.player_on_button:
            print(f"Player on button, is_pressed={self.is_button_pressed}, activated={self.already_activated}")
        
        # Simple button press state logic
        if self.player_on_button and not self.is_button_pressed:
            self.is_button_pressed = True
            self.image = self.button_pressed
        elif not self.player_on_button and self.is_button_pressed:
            self.is_button_pressed = False
            self.image = self.button_normal
            
        # Validate image is a surface
        if not isinstance(self.image, pygame.Surface):
            print(f"Warning: Button image is not a valid surface")
...
class InfoBubble(pygame.sprite.Sprite):
    ...
    def create_surface(self):
        ...
        # Validate image is a surface
        if not isinstance(self.image, pygame.Surface):
            print(f"Warning: Created invalid surface in InfoBubble")
            self.image = pygame.Surface((self.max_width + self.padding * 2, total_height))
            self.image.fill((200, 200, 200))  # Gray fallback
...
class StarrySky:
    def __init__(self, width, height, num_stars=100):
        ...
        self.height = height
        self.stars = [
            (random.randint(0, width), random.randint(0, height), random.randint(1, 3), (255, 255, 255))
...
class PlanetPlatformer:
    def __init__(self, planet_name, planet_color, planet_facts, background_color=(0, 0, 0)):
        ...
        self.starry_sky = StarrySky(WORLD_WIDTH, WORLD_HEIGHT)
    ...
    def setup_fact_bubble(self, button):
        # Set button cooldown
        button.cooldown = 30
    ...
    def check_button_collisions(self):
        for button in self.info_buttons:
            button.player_on_button = pygame.sprite.collide_rect(self.player, button)
            
            if button.player_on_button and button.cooldown == 0 and not button.already_activated:
                # Handle button activation logic
                self.setup_fact_bubble(button)
                button.already_activated = True
            elif not button.player_on_button:
                button.already_activated = False
    ...
    def run(self):
        ...
        while running:
            if self.game_state == "menu":
                ...
                # Draw everything
                self.starry_sky.draw(self.screen, self.camera.x)
